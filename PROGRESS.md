...
- **Branch:** `main` (synced with fork `erikgunawans/TradingAgents:main` at `fa99ae2`). **Today (2026-05-31): 6 PRs shipped** — TradiX rebrand (#34), i18n rating/status/chart-warning coverage (#35), deploy-prune `-af` fix (#36), favicon (#37), `types.ts` re-export refactor (#38), CI Node 20→24 bump (#39). Plus Google OAuth credentials provisioned + `/etc/tradingagents/env` cleaned. **All prod-verified live.** **⚠️ CI `playwright e2e` job is red on main** since PR #36; pre-existing rot (NOT from any of today's PRs) — likely from one of 3 unreviewed direct-to-main commits (`f071566` i18n toggle, `2c6a264` guest login, `7c35473` env-flag fix) that bypassed CI between the 2026-05-25 baseline and today. Deploy pipeline + prod app are healthy; CI is just stuck.
...
- **PRs merged:** 39 total — Waves 1-3 (#1-#3) + 12 v3+ followups (#4-#14) + worker hotfix (#15) + UI modernization (#16) + 2 Indonesia (#17-#18) + production deploy (#19) + PROGRESS checkpoint (#20) + deploy hot-fix (#21) + 3 Wave 4 (#22-#24) + 3 Wave 5 (#25-#27) + Wave 5.4 notifications (#28) + Resend→SendGrid pivot (#29) + 4 hardening (#30-#33) + **TradiX rebrand (#34)** + **i18n rating/status/chart coverage (#35)** + **deploy-prune -af fix (#36)** + **favicon (#37)** + **types.ts re-export refactor (#38)** + **CI Node 20→24 bump (#39)**.
...
| Metric | Value |
|---|---|
| Local main HEAD | `fa99ae2` (Merge PR #39 — CI Node 20→24 bump) |
| Most recent PR | #39 — `ci: bump GitHub Actions to Node 24 majors`, merged 2026-05-31 |
| VM disk | 5.8G / 30G (21%) steady-state — PR #36's `-af` prune fix verified holding across 5 deploys today |
...
- **🔴 Fix CI `playwright e2e` job (red since PR #36)** — pre-existing rot, NOT caused by any of today's PRs (deploys are all green; only the `ci.yml` workflow's e2e job fails). Symptom: `signInAs` helper times out — sign-in returns to `/login?callbackUrl=...` instead of redirecting to `/history`. 2 specs fail (`login.spec.ts:4:7`, `watchlist.spec.ts:11:7`); 24 pass / 6 skipped. Diagnosis order: (1) `gh run view <id> --log` of a failed CI run to see if `seed_e2e.py` actually ran and what env vars the dev server saw; (2) compare `web/lib/auth.ts` credentials provider against what the `signInAs` helper expects; (3) check whether `E2E_TEST_MODE` flag survived the guest-login env-flag refactor (`7c35473`). Most-likely cause: one of the 3 direct-to-main commits between the 2026-05-25 baseline and today (`f071566` i18n toggle, `2c6a264` guest login, `7c35473` env-flag fix) — those bypassed CI; the regression went unnoticed until today's PR pushes re-exercised the suite.
- **Unify Pydantic Run schemas to use `RunStatus` enum** — `RunOut.status` and `RunDetailOut.status` are typed as `str` server-side, but always one of the four enum values. PR #38's `WithRunStatus<T>` TS-side narrowing is a workaround; unifying the Pydantic schemas (RunTailOut already does) lets us drop the wrapper. Same pattern for `RunCreate.asset_type` (Pydantic default → openapi marks required → TS re-narrows to optional). Each is a small server edit + codegen.
...
## Checkpoint 2026-05-31 — late evening (PR #39 CI Node 24 bump + caught pre-existing CI e2e rot)

- **Session:** Continued from the 5-PR shipping session — added one more PR (CI Node 20→24 majors bump). Mid-PR, discovered CI's `playwright e2e` job has been red on every main run since PR #36, but missed earlier because pre-merge dispatches all targeted `deploy.yml` (green) rather than `ci.yml`.
- **Branch:** `main` @ `fa99ae2` (Merge PR #39), synced with fork.
- **Done:**
  - **PR #39 — CI Node 20→24 majors bump** (merge `fa99ae2`). Bumped `actions/checkout@v4`→`@v6` (3 sites: ci.yml ×2, deploy.yml ×1), `docker/setup-buildx-action@v3`→`@v4`, `docker/login-action@v3`→`@v4`, `docker/build-push-action@v6`→`@v7`. Eliminates the "Node 20 actions deprecated, forced to Node 24 by June 16, 2026" warning on every deploy. Self-validated: this PR's own pre-merge `deploy.yml` dispatch ran the bumped docker/* actions cleanly, no deprecation warning in the logs. All four majors use Node 24 as default runtime per their release notes; require Actions Runner ≥ v2.327.1 (GitHub-hosted are well past). Note: kept `actions/checkout@v6` (latest stable) rather than `@v5` (first Node-24-capable) — v6 adds per-file credential persistence (PR #2286) which is transparent to our usage.
  - **Caught pre-existing CI e2e rot.** While investigating why CI workflow run on this PR's branch failed, found that EVERY CI run on main today has been red. PR #39 is NOT the cause — the unit+typecheck+build job is green, but the playwright e2e job has been failing on `signInAs` (URL stuck at `/login?callbackUrl=...` instead of redirecting to `/history`). Pre-existing since PR #36. 2 specs fail (`login.spec.ts:4:7`, `watchlist.spec.ts:11:7`); 24 pass / 6 skipped. Logged as the new top-priority backlog item. Most likely root cause: one of the 3 direct-to-main commits (`f071566` i18n, `2c6a264` guest login, `7c35473` env-flag fix) that bypassed CI between the 2026-05-25 baseline and today, and the regression only surfaced when CI re-ran on today's PRs.
- **Files changed:** 2 — `.github/workflows/ci.yml` (2 sites), `.github/workflows/deploy.yml` (4 sites).
- **Tests:** unit+typecheck+build CI job green. e2e CI job red (24 pass / 6 skipped / 2 failed — the 2 failures are pre-existing rot, not from #39).
- **Findings worth carrying forward:**
  - **Pre-merge `deploy.yml` dispatches don't catch CI breakage** — only the `ci.yml` workflow (auto-triggered on PR push) does. When merging a PR with red CI, GitHub allows it (no required-checks gating). Worth adding a branch-protection rule that requires `ci.yml` green before merge to main.
  - **The `actions/checkout@v6` "Persist creds to a separate file" change** is technically a behavior change but transparent in our usage — we don't read git creds from subsequent steps. Future projects: watch out if anything reads `~/.git-credentials` or runs `git config user.email` after checkout; the path changed in v6.
- **Next:** Fix the CI e2e regression (top priority). Diagnosis order in the "What To Do Next" section above. Once green, optionally add a branch-protection rule requiring `ci.yml` to pass before merge so this class of "merged through red CI" can't recur.

---

## Checkpoint 2026-05-31 (5-PR shipping session — rebrand, i18n coverage, deploy hygiene, favicon, types refactor)

- **Session:** Resumed after long pause. User triggered work on each item in turn; each shipped end-to-end (PR → pre-merge deploy → smoke → merge → post-merge deploy → confirm) before moving on. Plus mid-session: Google OAuth credentials provisioned + env-propagation bug diagnosed and fixed.
- **Branch:** `main` @ `96ddae7` (Merge PR #38), synced with fork.
- **Done:**
  - **PR #34 — TradiX rebrand** (merge `1d6274c`). New Axiara brand mark (`web/public/logo-mark.png`, 66KB) replaces the inline SVG slash-mark in Nav.tsx. Wordmark "Trading|Agents" → "TradiX". Metadata titles across `layout.tsx` / `login` / `signals` / `watchlist` → "TradiX". Login eyebrow "tradingagents" → "tradix" + replaced placeholder `/` glyph with the actual logo.
  - **Google OAuth provisioning + env-propagation fix** (no PR, ops-only). Created OAuth Web client in GCP Console (project `tradix-axiara`); added `AUTH_GOOGLE_ID` + `AUTH_GOOGLE_SECRET` to `/etc/tradingagents/env` on the VM. **Caught a latent compose-file bug**: base `docker-compose.yml` had `environment: NEXTAUTH_SECRET: ${NEXTAUTH_SECRET}` shell-substitution stubs that shadow the prod overlay's `env_file:` for any same-named var. First container recreate dropped GITHUB + NEXTAUTH because shell was empty under sudo. Fix: `set -a; source /etc/tradingagents/env; set +a` before `docker compose up -d --force-recreate`. The compose-file stubs should be removed in a future cleanup so future env edits don't hit this trap.
  - **PR #35 — i18n rating/status/chart-warning coverage** (merge `785d4ca`). User reported "most of the content doesn't honor the language toggle." Audit found 3 components rendered raw English instead of going through `t()`: `RatingBadge` (Buy/Overweight/Hold/Underweight/Sell labels), `DecisionTimeline` (raw `{d.rating}` + `{d.status}` from API), `TickerChartWorkspace` (clipped-data warning). New catalog sections `rating.*` (Beli / Akumulasi / Tahan / Kurangi / Jual — matches Indonesian sell-side analyst convention per BRI Danareksa / Mandiri Sekuritas) + `decisionStatus.*` (menunggu / selesai) + `ticker.dataRangeClipped`. Also polished 5 stiff/calque translations in the existing ID catalog using `/Users/erikgunawansupriatna/Downloads/id-translation-polish.skill` (e.g. `monitor agentik` → `Monitor harian`; `tenang di hari yang semuanya HOLD` → `tidak ada email di hari semua HOLD`).
  - **PR #36 — deploy-prune `-af` fix** (merge `c6f7dff`). After #34/#35 the VM hit 100% disk (29G/30G). Root cause: `.github/workflows/deploy.yml` used `docker image prune -f` (only removes DANGLING images, leaves tagged-but-unused images that every deploy creates). ~50 deploys × 500MB = ~25GB accumulated. Fix: `-af` flag on both prunes + added a pre-pull prune (`docker image prune -af || true`) so failed deploys can't compound the disk-fill state into an unrecoverable loop. **Empirically verified holding**: 5 deploys later the VM is at 21% (5.8G/30G) steady-state with exactly 5 images (one per active container, zero accumulation).
  - **PR #37 — favicon + apple-touch-icon** (merge `6112bb3`). Uses Next.js App Router file convention: `app/icon.png` (64×64, 4.7KB, three-strokes red mark) + `app/apple-icon.png` (180×180, 13.5KB, full Axiara mark with A glyph). Next auto-injects `<link rel=icon>` + `<link rel=apple-touch-icon>` with size hints — confirmed in the rendered HTML head. Source images were 1024×1024 PNGs resized via `sips -z` (97% smaller payload).
  - **PR #38 — `types.ts` re-export refactor** (merge `96ddae7`). Drains the drift class permanently — every hand-coded interface in `web/lib/types.ts` that mirrors a Pydantic schema is now `components["schemas"]["X"]`. Two narrowings kept (documented inline): `WithRunStatus<T>` restores the literal union on `RunOut.status` / `RunDetailOut.status` (Pydantic types them as `str` — RunTailOut already uses the enum, asymmetry server-side); `RunCreate.asset_type` re-narrowed to optional because Pydantic gives it a default that openapi-typescript marks required. `RunStatus` and `AnalystKey` stay hand-defined (TS-only literal unions, no Pydantic counterpart). Net `-64 +40` lines despite added documentation. Vitest 21/21 + build clean.
- **Files changed across session:** 16+ files. New: `web/public/logo-mark.png`, `web/app/icon.png`, `web/app/apple-icon.png`. Modified: Nav.tsx, layout.tsx, login/page.tsx, signals/page.tsx, watchlist/page.tsx, RatingBadge.tsx, DecisionTimeline.tsx, TickerChartWorkspace.tsx, lib/i18n/messages/{en,id}.ts, web/lib/types.ts, .github/workflows/deploy.yml.
- **Tests:** 263 server + 21 vitest unchanged. CI green for every PR via the workflow merged in PR #33.
- **Findings worth carrying forward:** (1) The `docker-compose.yml` base file has `environment: VAR: ${VAR}` stubs that override `env_file:` in the prod overlay — silently nukes vars when shell env is empty (e.g. cron, fresh sudo). Future env edits must `set -a; source /etc/tradingagents/env; set +a` before recreate, OR clean up the stubs. (2) `docker compose restart` doesn't re-read `env_file:` (only on container *creation*) — must `--force-recreate` after env changes. (3) `docker image prune -f` is essentially a no-op for CI-deploy workflows that tag each artifact — always use `-af`. (4) Re-export typing via `Omit<T, "field"> & { field: NarrowedType }` is the canonical way to override one property of a generated type while preserving automatic field-tracking on the rest.
- **Next:** CI Node 20→24 (Sept 2026 deadline, now warned on every deploy as a June 16 2026 forcing date too). Unify Pydantic Run schemas to use the `RunStatus` enum (drops PR #38's TS narrowing). Provision SendGrid for live alerts (still operator-gated). The e2e suite's 6 skips (price fixture + RunsBadge rework).
