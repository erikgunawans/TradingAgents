...
- **Branch:** `main` (synced with fork `erikgunawans/TradingAgents:main` at `96ddae7`). **Today (2026-05-31): 5 PRs shipped** — TradiX rebrand (#34), i18n rating/status/chart-warning coverage (#35), deploy-prune `-af` fix (#36), favicon (#37), `types.ts` re-export refactor (#38). Plus Google OAuth credentials provisioned + `/etc/tradingagents/env` cleaned. **All prod-verified live.**
...
| Metric | Value |
|---|---|
| Local main HEAD | `96ddae7` (Merge PR #38 — types.ts re-exports) |
| Most recent PR | #38 — `refactor: re-export types from openapi-types`, merged 2026-05-31 |
...
- **CI Node.js 20 → 24 migration (deadline Sept 2026)** — workflow annotations warn Node 20 actions are deprecated. Bump `actions/checkout`, `docker/setup-buildx-action`, `docker/login-action`, `docker/build-push-action` to Node-24-compatible majors. Now also surfaced as a `June 16, 2026` warning on every deploy.
